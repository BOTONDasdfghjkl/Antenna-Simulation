function run_femm(dr_ohf,dr_femm,dr_data,f,CPW_thickness,dx,dy,dz,Nx,Nz)
    ...
    for ti = 1:length(CPW_thickness)
        ...
        p_left = [0,0
                  width_ground,0
                  width_ground,CPW_thickness(ti)
                  0,CPW_thickness(ti)];
        %második antenna source
        p_middle = [width_ground + width_gap,0
                    width_ground + width_signal + width_gap,0
                    width_ground + width_signal + width_gap,CPW_thickness(ti)
                    width_ground + width_gap,CPW_thickness(ti)];
        ...
        p_right = [width_ground + width_signal + 2.0*width_gap,0
                    ...
                    2*width_ground + width_signal + 2.0*width_gap,CPW_thickness(ti)
                    width_ground + width_signal + 2.0*width_gap,CPW_thickness(ti)];
        %Az alja a cuccnak
        ...
        mi_probdef(f,'micrometers','planar',1e-8,100,30,0);
        mi_saveas(char(strrep(cd +"\"+ dr_femm+"\CPW_design_"+ti+".fem", '\', '\\')));
        mi_analyze(1);
    end
    closefemm;
end
